...
- The Homebrew branch in `install.sh` is macOS/Linuxbrew; the pip/apt branches are the non-Homebrew fallback. The winget branch in `install.ps1` is the Windows equivalent; choco/pip are its fallback. Keep all paths working if you touch the dependency-install functions.
